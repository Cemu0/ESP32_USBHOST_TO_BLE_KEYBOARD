...
{
	xTaskCreate(this->taskServer, "server", 10000, (void *)this, 5, &TaskHandle_1);
	// if(killable == NULL)
 	// 	xTaskCreate(this->taskServer, "server", 20000, (void *)this, 5, &TaskHandle_1);
	// else
	// 	*killable = 0;
//   xTaskCreate(this->taskServer, "kserver", 10000, (void *)this, 5,NULL);
}
...
/*void BleKeyboard::taskServer(void* pvParameter) {
  BleKeyboard* bleKeyboardInstance = (BleKeyboard *) pvParameter; //static_cast<BleKeyboard *>(pvParameter);
	//https://github.com/nkolban/esp32-snippets/issues/630#issuecomment-454834560
  nvs_flash_init();
...
//   BLEServer *pServer = _pServer;
  pServer->setCallbacks(bleKeyboardInstance->connectionStatus);
  bleKeyboardInstance->killable = new int(0);
  BLEHIDDevice *BHD = new BLEHIDDevice(pServer);
  bleKeyboardInstance->hid = BHD;
  ...
  bleKeyboardInstance->outputKeyboard->setCallbacks(new KeyboardOutputCallbacks()); //cant delete 
...
//   pAdvertising->setScanFilter(false, true);
  ...
  ESP_LOGD(LOG_TAG, "Advertising started!");
	...
	while(!*bleKeyboardInstance->killable)
	{
		vTaskDelay(100);
	}	
  ...
  *bleKeyboardInstance->killable = -1;
  Serial.println("Task_released");

	while(*bleKeyboardInstance->killable == -1)
	{
		vTaskDelay(100);
	}	
}
  //can't be deleted , unkown why?
//   delete pAdvertising;
//   delete pSecurity;
//   delete bleKeyboardInstance->hid;
//   delete pServer;
//   *bleKeyboardInstance->killable = -1;
//   Serial.println("Task_released");
//   vTaskDelay(100);
//   delete bleKeyboardInstance;
//   killAble = -1;;
...
BLEHIDDevice *BHD;
KeyboardOutputCallbacks *KOC = new KeyboardOutputCallbacks();
bleKeyboardInstance->kocb = KOC;
BLESecurity *pSecurity;
BLEAdvertising *pAdvertising = NULL;
bleKeyboardInstance->killable = new int(0);
while(1)
{
//   bleKeyboardInstance = (BleKeyboard *) pvParameter; //static_cast<BleKeyboard *>(pvParameter);
	//https://github.com/nkolban/esp32-snippets/issues/630#issuecomment-454834560
//   nvs_flash_init();
...
//   BLEServer *pServer = _pServer;
  ...
  bleKeyboardInstance->connectionStatus->outputKeyboard = bleKeyboardInstance->outputKeyboard;
  bleKeyboardInstance->connectionStatus->inputMediaKeys = bleKeyboardInstance->inputMediaKeys;
//   KOC =	new KeyboardOutputCallbacks();
//   bleKeyboardInstance->kocb = KOC;
  bleKeyboardInstance->outputKeyboard->setCallbacks(bleKeyboardInstance->kocb);
  ...
  ESP_LOGE(LOG_TAG, "Advertising started!");
  ...
  ESP_LOGD(LOG_TAG, "Advertising started!"); 
	...
	{
		vTaskDelay(100);
		if(bleKeyboardInstance->connectionStatus->connected && !stoped)
		{
			//for saving energy ? about 40mah ?
			pAdvertising->stop();
			stoped = true;
